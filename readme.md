...
> Upstream baseline documentation. This fork's Demo setup, profiles, safety
> behavior, and benchmark instrumentation are documented in [NEXT.md](NEXT.md).
...
This project is based on the Blender Foundation's official Blender MCP server
and add-on (baseline tag `v1.0.0`), which are copyright Blender Authors and
distributed under GPL-3.0-or-later. All modifications and additions (the
agent-native structured operation layer, capability packs, metrics, and
documentation) are likewise licensed GPL-3.0-or-later.
